...
module Vouch
  class PendingAuthentication
    def self.fingerprint(account)
      ...
      Digest::SHA256.hexdigest([account.class.name, account.id, account.try(:created_at)&.utc&.iso8601(6),
    ...
    def self.build(account, identities:, method:, hook:, provider: nil, oauth: nil)
      {
        ...
        'identity_ids' => identities.map { |identity| identity.id.to_s },
        'method' => method.to_s,
        'hook' => hook.to_s,
        'provider' => provider,
        'oauth' => oauth
    ...
    def self.valid?(context, account)
      ...
      return false unless context['account_id'] == account.id.to_s
