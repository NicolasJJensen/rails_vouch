module Vouch
  module InvitationHelpers
    ...
    def invitation_requires_registration?(invitation)
      invitation.invitation_registration_required? && auth_mapping.account_for(invitation).registration_required?
    end

    def accept_pending_invitation(account, publish: true)
      invitation = pending_invited_identity
      return nil unless invitation && auth_mapping.account_for(invitation) == account
      ...
      accepted = account.with_lock do
        invitation.with_lock do
          ...
          next false unless auth_mapping.account_for(invitation) == account
