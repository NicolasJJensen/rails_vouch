...
RSpec.describe "Users lifecycle hook phases", type: :request do
  ...
  it "runs sign-up commit hooks after persistence and identity publication" do
    observed = []
    preserve_hooks(Users::RegistrationsController, :sign_up) do
      Users::RegistrationsController.after_commit_of_sign_up do |account, identity|
  ...
  it "runs OAuth account-creation commit hooks after the identity is signed in" do
    observed = []
    ...
    preserve_hooks(Users::OmniAuthsController, :oauth_account_creation) do
      Users::OmniAuthsController.after_commit_of_oauth_account_creation do |_account, identity|
  ...
  it "runs sign-out commit hooks after Warden has removed the identity" do
    user = create(:user)
    observed = []
    preserve_hooks(Users::SessionsController, :sign_out) do
      Users::SessionsController.after_commit_of_sign_out { observed << current_identity }
      sign_in(user)
  ...
  it "runs impersonation commit hooks after the target becomes current" do
    ...
    preserve_hooks(Users::ImpersonationsController, :impersonation_start) do
      Users::ImpersonationsController.after_commit_of_impersonation_start do |_original, target_identity|
