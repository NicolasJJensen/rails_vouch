...
RSpec.describe "Users::Passwords", type: :request do
  ...
  describe "GET /users/password_reset/new" do
    it "renders the forgot password form" do
      get "/users/password_reset/new"
  ...
  describe "POST /users/password_reset" do
    it "redirects to sign-in with a notice (existing email)" do
      post "/users/password_reset", params: { email_address: "reset@example.com" }
      expect(response).to redirect_to("/users/sign_in")
    end

    it "redirects to sign-in with the same notice (non-existent email)" do
      post "/users/password_reset", params: { email_address: "nobody@example.com" }
      expect(response).to redirect_to("/users/sign_in")
    end

  end

  describe "GET /users/password_reset/edit" do
    it "renders the reset form with a valid token" do
      token = account.generate_password_reset_token!.value
      get "/users/password_reset/edit", params: { token: token }
      expect(response).to have_http_status(:ok)
    end

    it "redirects for an invalid token" do
      get "/users/password_reset/edit", params: { token: "invalid" }
      expect(response).to redirect_to("/users/sign_in")
    end
    ...
    it "redirects for array and nested token parameters" do
      ...
      get "/users/password_reset/edit", params: { token: { nested: "invalid" } }
      expect(response).to redirect_to("/users/sign_in")
    end
  ...
  describe "PATCH /users/password_reset" do
    it "updates the password with a valid token" do
      token = account.generate_password_reset_token!.value
      patch "/users/password_reset", params: {
        token: token,
