...
RSpec.describe Vouch::Generators::OmniauthGenerator do
  ...
  it "defaults the owner scope to accounts" do
    argument = described_class.arguments.find { |candidate| candidate.name.to_s == "scope" }

    expect(argument.default).to eq("accounts")
  end

  it "generates a non-polymorphic Account-owned identity and wires the account" do
    ...
    expect(model).to include("belongs_to :account")
    expect(model).not_to include("polymorphic")
    expect(account).to include('has_many :oauth_identities, class_name: "OauthIdentity"')
    expect(account).to include("foreign_key: :account_id, dependent: :destroy")
    expect(migration).to include("t.references :account, null: false, foreign_key: { to_table: :accounts }, type: :uuid")
  ...
  it "uses the selected single-model owner for its identity and association" do
    ...
    expect(member).to include("foreign_key: :member_id, dependent: :destroy")
    expect(migration).to include("t.references :member, null: false, foreign_key: { to_table: :members }")
  end
